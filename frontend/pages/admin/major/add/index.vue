...
import type { MajorRequest } from '~/types/MajorRequest'
...
// const { data: major, refresh } = await useFetch<Major>(`${apiBase}/major/${route.params.id}`)

const { data: branchs, error } = await useFetch(`${apiBase}/branch/mini`)

const { data: degrees } = await useFetch<Degree[]>(`${apiBase}/degree`)

const { data: programs } = await useFetch<Program[]>(`${apiBase}/program`)

const filterProgram = computed(() => {
    return programs.value?.filter(p => p.degree.id === degreeId.value)
})
...
const careersComputed = computed<string[]>({
    ...
    set(newArray: string[]) {
        if (major.value) {
            major.value.careers = newArray.join(',')
        }
    }
})
...
async function submitForm() {
    ...
    formData.append('programId', String(major.value.programId))
    formData.append('nameTh', major.value.nameTh)
    formData.append('nameEn', major.value.nameEn)
    formData.append('tuitionFees', String(major.value.tuitionFees))
    formData.append('language', major.value.language)
    formData.append('description', major.value.description)
    formData.append('careers', major.value.careers)
    formData.append('status', major.value.status)
...
<template>
    <div class="container mx-auto p-4">
        ...
        <div class="relative w-full aspect-[16/6] rounded-2xl shadow-lg overflow-hidden text-white">
            <NuxtImg v-if="major?.image" :src="previewURL" width="1920" height="1080"
                format="webp" sizes="(max-width: 1080px) 100vw, 1080px" alt="`ภาพปกสาขาวิชา ${major?.nameTh}`" priority
                class="object-cover" />
            ...
            <div class="lg:col-span-1 space-y-6">
                <Card>
                    ...
                    <CardContent class="flex flex-col gap-3">
                        <Label>TH</Label>
                        <Input type="text" v-model="major.nameTh" placeholder="ชื่อสาขาวิชาภาษาไทย" />
                        <Label>EN</Label>
                        <Input type="text" v-model="major.nameEn" placeholder="ชื่อสาขาวิชาภาษาอังกฤษ" />
                    </CardContent>
                </Card>
            </div>
                    ...
                    <ul class="space-y-4">
                        <li class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <Select v-model:model-value="degreeId">
                                    <SelectTrigger>
                        ...
                        <li class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <Select v-model:model-value="major.programId">
                                    <SelectTrigger>
                                        <SelectValue placeholder="Select a program" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectGroup>
                                            <SelectItem v-for="program in filterProgram" :key="program.id"
                                                :value="program.id">
                                                {{ program.nameTh }}
                                            </SelectItem>
                                        </SelectGroup>
                        ...
                        <li class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <Select v-model:model-value="major.language">
                                    <SelectTrigger>
                        ...
                        <li class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <NumberField v-model="major.tuitionFees" id="number" :default-value="0" :format-options="{
            ...
            <div class="lg:col-span-2 space-y-6">
                <div
                    ...
                    <div>
                        ...
                        <Textarea v-model.lazy="major.description" placeholder="Type your message here." />
