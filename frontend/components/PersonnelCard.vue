      ...
      <CardContent class="p-6 text-center flex flex-col h-full">
        <div class="flex-grow">
          <Avatar class="w-28 h-28 mx-auto mb-4 border-4 border-white dark:border-gray-700 shadow-lg">
            <AvatarImage :src="`${apiBase}/images/${imageUrl}`" :alt="name" class="object-cover" />
          ...
          <ul v-if="displayedSecondaryPositions.length > 0" class="mt-2 text-xs text-gray-500 dark:text-gray-400 space-y-1">
            <li v-for="pos in displayedSecondaryPositions" :key="pos.id" class="flex items-center justify-center gap-1.5">
          ...
          <div v-if="major" class="flex flex-wrap justify-center gap-2">
            <Badge
              variant="secondary"
